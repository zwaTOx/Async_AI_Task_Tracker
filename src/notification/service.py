...
class NotificationService:
    ...
    async def create_notification(self, notif_data: NotificationCreate):
        new_notif = await NotificationRepository(self.session).create(notif_data)
        return new_notif
    
    async def create_project_notification(self, user_id: int, project_id: int, notif_data: ProjectNotificationCreate):
